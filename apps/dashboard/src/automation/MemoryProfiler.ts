...
export class MemoryProfiler {
  ...
  stopProfiling = async (): Promise<MemoryData> => {
    ...
    try {
      ...
      if (this.options.trackAllocationSampling) {
        const samplingProfile = await this.cdpSession.send('HeapProfiler.stopSampling');
      ...
      this.isRecording = false;
      this.log(`Memory profiling completed. Collected ${this.heapSnapshots.length} snapshots, ${this.gcEvents.length} GC events`);
   ...
   */
  captureHeapSnapshot = async (label: string = 'manual'): Promise<MemorySnapshot> => {
    if (!this.isRecording) {
      throw new Error('Memory profiling not active');
    }

    if (this.heapSnapshots.length >= this.options.maxSnapshots) {
      this.log(`Maximum snapshots (${this.options.maxSnapshots}) reached, skipping capture`);
    ...
    try {
      ...
      this.log(`Heap snapshot captured: ${label} (${memorySnapshot.metadata.nodeCount} nodes)`);
  ...
  private setupEventListeners = (): void => {
    ...
    const consoleListener = (params: any) => {
      if (params.type === 'profile' && params.args?.[0]?.value?.includes('memory')) {
  ...
  private startUsageMonitoring = (): void => {
    ...
    this.log(`Started usage monitoring (${this.options.usageMonitoringInterval}ms intervals)`);
  };
    ...
    try {
      ...
      if (result.result?.value) {
        ...
        const usage: HeapUsageMetric = {
          timestamp: Date.now() - this.recordingStartTime,
          source,
          usedJSHeapSize: memoryInfo.performance?.usedJSHeapSize || memoryInfo.memoryInfo?.used || 0,
          totalJSHeapSize: memoryInfo.performance?.totalJSHeapSize || memoryInfo.memoryInfo?.total || 0,
          jsHeapSizeLimit: memoryInfo.performance?.jsHeapSizeLimit || memoryInfo.memoryInfo?.limit || 0,
  ...
  private handleGCEvent = (params: any): void => {
    ...
    this.log(`GC event recorded: ${gcEvent.type} (${gcEvent.duration}ms, freed ${gcEvent.freedBytes} bytes)`);
  };
  ...
  private calculateMemoryPressure = (memoryInfo: any): number => {
    ...
    const used = memoryInfo.performance?.usedJSHeapSize || memoryInfo.memoryInfo?.used || 0;
    const total = memoryInfo.performance?.totalJSHeapSize || memoryInfo.memoryInfo?.total || 0;
  ...
  private analyzeMemoryData = (): MemoryAnalysis => {
    ...
    const avgMemoryUsage = this.heapUsageHistory.length > 0
      ? this.heapUsageHistory.reduce((sum, usage) => sum + usage.usedJSHeapSize, 0) / this.heapUsageHistory.length
      : 0;
    ...
    return {
      totalGCTime,
      avgMemoryUsage,
      maxMemoryUsage: Math.max(...this.heapUsageHistory.map(u => u.usedJSHeapSize)),
      memoryGrowthRate: memoryGrowth.rate,
      memoryGrowthTrend: memoryGrowth.trend,
      gcFrequency: this.gcEvents.length / (Date.now() - this.recordingStartTime) * 1000, // events per second
      gcEfficiency,
   ...
   */
  private calculateMemoryGrowth = (): { rate: number; trend: 'increasing' | 'decreasing' | 'stable' } => {
  ...
  private detectPotentialLeaks = (): ObjectRetention[] => {
    ...
    if (this.heapSnapshots.length >= 2) {
      ...
      if (sizeIncrease > 1024 * 1024) { // More than 1MB increase
        suspects.push({
          objectType: 'unknown',
  ...
  private calculateGCEfficiency = (): number => {
    ...
    const totalFreed = this.gcEvents.reduce((sum, event) => sum + event.freedBytes, 0);
   ...
   */
  private generateRecommendations = (leaks: ObjectRetention[], gcEfficiency: number): string[] => {
    const recommendations: string[] = [];

    if (leaks.length > 0) {
      recommendations.push('Potential memory leaks detected - review object retention patterns');
    }

    if (gcEfficiency < 1000) { // Less than 1KB freed per ms
      recommendations.push('Low GC efficiency - consider reducing object allocations');
    }

    if (this.gcEvents.length > 10) {
      recommendations.push('High GC frequency - optimize memory usage patterns');
    }

    const avgPressure = this.heapUsageHistory.reduce((sum, u) => sum + u.memoryPressure, 0) / this.heapUsageHistory.length;
    if (avgPressure > 0.8) {
      recommendations.push('High memory pressure - consider memory optimization');
    }
