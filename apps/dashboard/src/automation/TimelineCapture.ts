...
export class TimelineCapture {
  ...
  stopRecording = async (): Promise<TimelineData> => {
    ...
    try {
      ...
      this.isRecording = false;
      this.log(
        `Timeline capture completed. Collected ${this.rawEvents.length} events`
      );
   ...
   */
  private processTimelineData = async (
    finalMetrics: any
  ...
  private shouldIncludeEvent = (
    event: CDPPerformanceTimelineEvent
  ): boolean => {
   ...
   */
  private calculateCoreWebVitals = async (
    finalMetrics: any,
    ...
    const firstContentfulPaint = paintEvents.find((e) =>
      e.name.includes("first-contentful-paint")
    );
    ...
    return {
      ...
      fid: finalMetrics.fid
        ? {
            value:
              finalMetrics.fid.processingStart - finalMetrics.fid.startTime,
      ...
      cls:
        finalMetrics.layoutShift && Array.isArray(finalMetrics.layoutShift)
          ? {
              value: finalMetrics.layoutShift.reduce(
      ...
      // TODO: Implement INP (Interaction to Next Paint)
   ...
   */
  private extractNavigationTiming = (
    finalMetrics: any
  ): NavigationTimingMetrics => {
  ...
  private analyzeJavaScriptPerformance = (
    ...
    const totalExecutionTime = scriptEvents.reduce(
    ...
    return {
      totalExecutionTime,
      mainThreadBlockingTime: longTasks.reduce(
  ...
  private analyzeLayoutPaint = (
    ...
    const firstPaint = finalMetrics.paint?.find(
      (p: any) => p.name === "first-paint"
    );
    const firstContentfulPaint = finalMetrics.paint?.find(
      (p: any) => p.name === "first-contentful-paint"
    );

    return {
      layoutCount: layoutEvents.length,
      totalLayoutTime: layoutEvents.reduce(
        (sum, event) => sum + (event.duration || 0),
        0
      ),
      paintCount: paintEvents.length,
      totalPaintTime: paintEvents.reduce(
        (sum, event) => sum + (event.duration || 0),
        0
      ),
      ...
      forcedReflowCount: layoutEvents.filter((e) => e.name.includes("forced"))
        .length,
      avgLayoutTime:
