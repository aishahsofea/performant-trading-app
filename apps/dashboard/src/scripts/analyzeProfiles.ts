...
class DevToolsAnalyzer {
  ...
  private async findLatestResultsFile(): Promise<string> {
    try {
      ...
      const resultFiles = files
        .filter(f => f.startsWith('devtools-') && f.endsWith('.json') && !f.includes('analysis-report'))
        .sort()
        .reverse();

      if (resultFiles.length === 0) {
        throw new Error('No DevTools results files found. Run devtools:capture first.');
      }

      return join(this.outputDir, resultFiles[0]!);
    } catch (error) {
      throw new Error(`Could not find results files in ${this.outputDir}: ${error}`);
    }
  }
  ...
  private async loadBaseline(): Promise<any> {
    ...
    try {
      ...
      console.warn('⚠️  No baseline found for comparison');
      return null;
    }
  }

  private generateAnalysisReport(results: TestSuiteResult, baseline: any): AnalysisReport {
    ...
    const averageLoadTime = successfulResults.length > 0 ? totalLoadTime / successfulResults.length : 0;
  ...
  private analyzeScenario(result: any): ScenarioAnalysis {
    const scriptTime = result.metrics.timeline?.javascript?.totalExecutionTime || 0;
    const layoutTime = result.metrics.timeline?.layoutPaint?.totalLayoutTime || 0;
    const memoryUsage = result.metrics.memory?.heapUsage?.length ? 
      Math.max(...result.metrics.memory.heapUsage.map((h: any) => h.usedJSHeapSize)) / (1024 * 1024) : 0;
    const totalTime = result.duration;

    const grade = this.calculatePerformanceGrade(scriptTime, layoutTime, memoryUsage, totalTime);
    const recommendations = this.generateRecommendations(scriptTime, layoutTime, memoryUsage);
  ...
  private generateRecommendations(scriptTime: number, layoutTime: number, memoryUsage: number): string[] {
    const recommendations: string[] = [];

    if (scriptTime > 200) {
      recommendations.push('Consider code splitting and lazy loading to reduce JavaScript execution time');
    ...
    if (layoutTime > 100) {
      ...
      recommendations.push('Consider optimizing CSS for better layout performance');
    }

    if (memoryUsage > 100) {
      recommendations.push('Investigate potential memory leaks and optimize object lifecycle');
    } else if (memoryUsage > 50) {
      recommendations.push('Monitor memory usage patterns for optimization opportunities');
    }
  ...
  private generateScenarioInsights(result: any, analysis: ScenarioAnalysis): PerformanceInsight[] {
    const insights: PerformanceInsight[] = [];
  ...
  private generateOverallInsights(results: TestSuiteResult): PerformanceInsight[] {
    const insights: PerformanceInsight[] = [];
  ...
  private compareWithBaseline(current: TestSuiteResult, baseline: any): any {
    ...
    for (const currentResult of current.results.filter(r => r.success)) {
      const baselineScenario = baseline.scenarios?.find((s: any) => s.scenario === currentResult.scenario);
      
      if (baselineScenario) {
        const currentScript = currentResult.metrics.timeline?.javascript?.totalExecutionTime || 0;
        const baselineScript = baselineScenario.scriptTime || 0;
        
        const scriptDiff = currentScript - baselineScript;
        const scriptPercent = baselineScript > 0 ? (scriptDiff / baselineScript) * 100 : 0;

        if (scriptPercent < -10) {
          improvements.push(`${currentResult.scenario}: Script time improved by ${Math.abs(scriptPercent).toFixed(1)}%`);
        } else if (scriptPercent > 20) {
          regressions.push(`${currentResult.scenario}: Script time regressed by ${scriptPercent.toFixed(1)}%`);
        }

        const currentMemory = currentResult.metrics.memory?.heapUsage?.length ? 
          Math.max(...currentResult.metrics.memory.heapUsage.map((h: any) => h.usedJSHeapSize)) : 0;
        const baselineMemory = baselineScenario.memoryPeak || 0;
        
        const memoryDiff = currentMemory - baselineMemory;
        const memoryPercent = baselineMemory > 0 ? (memoryDiff / baselineMemory) * 100 : 0;

        if (memoryPercent < -10) {
          improvements.push(`${currentResult.scenario}: Memory usage improved by ${Math.abs(memoryPercent).toFixed(1)}%`);
        } else if (memoryPercent > 25) {
          regressions.push(`${currentResult.scenario}: Memory usage regressed by ${memoryPercent.toFixed(1)}%`);
        }
      }
    }
  ...
  private calculateOverallHealth(analyses: ScenarioAnalysis[]): 'excellent' | 'good' | 'concerning' | 'poor' {
    if (analyses.length === 0) return 'poor';

    const gradePoints = { 'A': 4, 'B': 3, 'C': 2, 'D': 1, 'F': 0 };
    const totalPoints = analyses.reduce((sum, a) => sum + gradePoints[a.grade], 0);
  ...
  private async outputReport(report: AnalysisReport): Promise<void> {
    ...
    const reportFile = join(this.outputDir, `analysis-report-${timestamp}.json`);
  ...
  private printConsoleReport(report: AnalysisReport): void {
    ...
    console.log(`\n🎯 Summary:`);
    console.log(`  Health: ${this.getHealthEmoji(report.summary.overallHealth)} ${report.summary.overallHealth.toUpperCase()}`);
    console.log(`  Scenarios: ${report.summary.passedScenarios}/${report.summary.totalScenarios} passed`);
    console.log(`  Avg Load Time: ${report.summary.averageLoadTime.toFixed(0)}ms`);

    console.log(`\n📈 Scenario Performance:`);
    report.scenarios.forEach(s => {
      const gradeEmoji = this.getGradeEmoji(s.grade);
      console.log(`  ${gradeEmoji} ${s.name}: Grade ${s.grade}`);
      console.log(`     Script: ${s.performance.scriptTime}ms | Layout: ${s.performance.layoutTime}ms | Memory: ${s.performance.memoryUsage.toFixed(1)}MB`);
    });

    const criticalInsights = report.insights.filter(i => i.type === 'error' || i.type === 'warning');
