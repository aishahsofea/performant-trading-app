...
export const useDashboardLayout = () => {
  ...
  const [currentLayout, setCurrentLayout] =
    useState<LayoutConfiguration | null>(null);
  ...
  const fetchPreferences = useCallback(async () => {
    ...
    try {
      ...
      if (prefsData.currentLayoutId) {
        const currentLayoutData = layouts.find(
          (l) => l.id === prefsData.currentLayoutId
        );
  ...
  const saveLayout = useCallback(
    async (layoutData: SaveLayoutRequest) => {
      try {
        const response = await fetch("/api/dashboard/layouts", {
          method: "POST",
  ...
  const updateLayout = useCallback(
    async (id: string, layoutData: Partial<SaveLayoutRequest>) => {
      try {
    ...
    async (layoutId: string) => {
      try {
        ...
        if (selectedLayout) {
          ...
          setPreferences((prev) =>
            prev ? { ...prev, currentLayoutId: layoutId } : null
          );
        }
  ...
  const updatePreferences = useCallback(
    async (newPreferences: Partial<UserDashboardPreferences>) => {
      try {
        const response = await fetch("/api/dashboard/preferences", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const autoSaveLayout = useCallback(
    async (layout: LayoutConfiguration) => {
