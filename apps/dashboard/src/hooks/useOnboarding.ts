...
export const useOnboarding = () => {
  ...
  const fetchProgress = useCallback(async () => {
    ...
    try {
      ...
      setOnboardingState({
        ...
        completedTours: Array.isArray(data.completedTours) ? data.completedTours : [],
  ...
  const skipStep = useCallback(async (stepId: string) => {
    try {
      ...
      if (onboardingState) {
        setOnboardingState(prev => prev ? {
          ...prev,
          skippedSteps: [...prev.skippedSteps.filter(s => s !== stepId), stepId],
  ...
  const nextStep = useCallback(() => {
    ...
    const currentIndex = defaultSteps.findIndex(step => step.id === onboardingState.currentStepId);
  ...
  const setCurrentStep = useCallback(async (stepId: string) => {
    try {
      ...
      // Update local state
      if (onboardingState) {
        setOnboardingState(prev => prev ? { ...prev, currentStepId: stepId } : null);
      }
  ...
  // Update preferences
  const updatePreferences = useCallback(async (preferences: Partial<OnboardingState>) => {
    try {
      const response = await fetch('/api/onboarding/progress', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      // Update local state
      if (onboardingState) {
        setOnboardingState(prev => prev ? { ...prev, ...preferences } : null);
      }
  ...
  const getCurrentStep = useCallback(() => {
    ...
    return defaultSteps.find(step => step.id === onboardingState.currentStepId) || null;
  }, [onboardingState]);
  ...
  const shouldShowOnboarding = useCallback(() => {
    return (
      ...
      (!onboardingState || (!onboardingState.isComplete && onboardingState.progress < 100))
    );
