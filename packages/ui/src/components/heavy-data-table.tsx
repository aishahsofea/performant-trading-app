...
const HeavyDataTable: React.FC<HeavyDataTableProps> = ({
  ...
  const [sortColumn, setSortColumn] =
    React.useState<keyof TradingPosition>("symbol");
  ...
  const renderCell = (
    ...
    const formatValue = (val: any, k: keyof TradingPosition) => {
      if (k === "entryPrice" || k === "currentPrice")
      ...
      if (k === "timestamp")
        return new Date(val as number).toLocaleTimeString();
      return val;
    };
        ...
        <table className="w-full">
          ...
          <tbody className="bg-gray-900">
            {displayData.map((position, index) => (
              ...
              >
                {columns.map((column) =>
                  renderCell(position, column.key, index)
                )}
              </tr>
            ))}
          </tbody>
