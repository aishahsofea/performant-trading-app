...
import type { BundleAnalyzerResult, BundleData, PlanetData, SolarSystemData, BundleType, PerformanceRating } from '../types/solarSystem';

/**
 * Parse webpack bundle analyzer results into Solar System data structure
 */
export const parseBundleAnalyzerData = (analyzerData: BundleAnalyzerResult): BundleData[] => {
 ...
 */
export const createSolarSystemFromBundles = (bundles: BundleData[]): SolarSystemData => {
  ...
  sortedBundles.forEach((bundle, index) => {
    const planet = createPlanetFromBundle(bundle, index, totalSize, sortedBundles.length);
    planets.push(planet);
  });
 ...
 */
const createPlanetFromBundle = (bundle: BundleData, index: number, totalSize: number, totalBundles: number): PlanetData => {
  const sizeRatio = bundle.size / totalSize;
  
  // Add some randomness but keep it deterministic based on bundle name
  const seedHash = bundle.name.split('').reduce((a, b) => a + b.charCodeAt(0), 0);
...
const determineBundleType = (filename: string): BundleType => {
  ...
  if (filename.indexOf('vendor') !== -1 || filename.indexOf('node_modules') !== -1) return 'vendor';
  if (filename.indexOf('chunk') !== -1) return 'chunk';
  if (filename.indexOf('main') !== -1 || filename.indexOf('app') !== -1) return 'main';
  return 'asset';
};
 ...
 */
export const calculatePerformanceImpact = (bundleSize: number, phase: number): {
  score: number;
